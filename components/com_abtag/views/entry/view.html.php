...
class AbtagViewEntry extends JViewLegacy
{
	...
	public function display($tpl = null)
	{
		...
		$item->readmore_link = JRoute::_(AbtagHelperRoute::getEntryRoute($item->slug, $item->tag_ids, $item->language));
		...
		$dispatcher->trigger('onContentPrepare', array ('com_abtag.entry', &$item, &$item->params, $offset));

		$item->event = new stdClass;
		$results = $dispatcher->trigger('onContentAfterTitle', array('com_abtag.entry', &$item, &$item->params, $offset));
		$item->event->afterDisplayTitle = trim(implode("\n", $results));

		//Trigger event as com_content.article in order to retrieve the fields linked to the article.
		$results = $dispatcher->trigger('onContentBeforeDisplay', array('com_content.article', &$item, &$item->params, $offset));
		$item->event->beforeDisplayContent = trim(implode("\n", $results));

		$results = $dispatcher->trigger('onContentBeforeDisplay', array('com_abtag.entry', &$item, &$item->params, $offset));
		$item->event->beforeDisplayContent .= trim(implode("\n", $results));

		$results = $dispatcher->trigger('onContentAfterDisplay', array('com_abtag.entry', &$item, &$item->params, $offset));
	...
	protected function _prepareDocument()
	{
		...
		if($menu && ($menu->query['option'] !== 'com_abtag' || $menu->query['view'] !== 'entry' || $id != $this->item->entry_id))
		{
