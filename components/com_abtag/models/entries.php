...
class AbtagModelEntries extends JModelList
{
	...
	protected function populateState($ordering = 'ordering', $direction = 'ASC')
	{
		...
		$orderCol = $app->input->get('filter_order', 'tm.ordering');
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':' . serialize($this->getState('filter.tag_id'));
	...
	protected function getListQuery()
	{
		...
		$query->select(
			$this->getState(
			  'list.select',
			  'e.id AS entry_id, e.article_id, e.main_tag_id, tm.tag_id, a.id, a.title, a.alias, a.introtext, a.fulltext, ' .
		...
		$query->from('#__abtag_entry AS e')
		      ->join('INNER', '#__content AS a ON a.id=e.article_id')
		      ->join('LEFT', '#__abtag_entry_tag_map AS tm ON e.id=tm.entry_id');

		$params = $this->getState('params');
		$orderby_sec = $params->get('orderby_sec');

		// Join over the frontpage articles if required.
		if ($this->getState('filter.frontpage', false))
		{
		  // Join over the tags.
		  $query->select('t.path AS tag_route')
			->join('LEFT', '#__tags AS t ON t.id = e.main_tag_id');
		  // To prevent duplicates.
		  $query->group('e.id');

		  if ($orderby_sec === 'front')
		  {
		    $query->join('INNER', '#__content_frontpage AS fp ON fp.content_id = e.article_id');
		  }
		  else
		  {
		...
		{
			$query->join('LEFT', '#__content_frontpage AS fp ON fp.content_id = e.article_id');
		}
		...
		if (JPluginHelper::isEnabled('content', 'vote'))
		{
			...
			$query->select('COALESCE(NULLIF(ROUND(v.rating_sum  / v.rating_count, 0), 0), 0) AS rating, 
				...
				->join('LEFT', '#__content_rating AS v ON e.article_id = v.content_id');
		}
		...
		if (is_numeric($articleId))
		{
			...
			$query->where('e.id ' . $type . (int) $articleId);
		}
		elseif (is_array($articleId))
		{
			...
			$query->where('e.id ' . $type . ' (' . $articleId . ')');
		}

		// Filter by a single or group of tags
		$tagId = $this->getState('filter.tag_id');

		if (is_numeric($tagId) && !$this->getState('filter.frontpage', false))
		{
		  // Add subtag check
		  $includeSubTags = $this->getState('filter.subtags', false);

		  if($includeSubTags) {
		    $model = $this->getInstance('Tag', 'AbtagModel');
		    $childrenIds = $model->getChildren(true);
		  }

		  if(isset($childrenIds) && !empty($childrenIds)) {
		    $query->where('tm.tag_id IN('.(int)$tagId.','.implode(',', $childrenIds).')')
			  ->group('e.id');
		  }
		  else {
		    $query->where('tm.tag_id='.(int)$tagId);
		  }
		}
...
//echo $query;
	...
	public function getItems()
	{
		...
		foreach ($items as &$item)
		{
			...
			if (!$guest)
			{
				$asset = 'com_content.article.' . $item->article_id;
			...
			if ($item->params->get('show_tags'))
			{
				...
				$item->tags->getItemTags('com_abtag.entry', $item->entry_id);
			}

			if ($item->params->get('show_associations'))
			{
				$item->associations = ContentHelperAssociation::displayAssociations($item->article_id);
			}
		}
